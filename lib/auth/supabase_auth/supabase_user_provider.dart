...
class SemsSupabaseUser extends BaseAuthUser {
  SemsSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> semsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SemsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
